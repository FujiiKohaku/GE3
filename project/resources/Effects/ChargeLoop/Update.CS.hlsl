...
void main(uint32_t3 DTid : SV_DispatchThreadID)
{
    ...
    if (distanceToEmitter > 0.001f)
    {
        gParticles[particleIndex].velocity +=
            toEmitter / distanceToEmitter *
    ...
    float4 blue = float4(0.25f, 0.75f, 1.0f, 0.85f);
    float4 purple = float4(0.65f, 0.3f, 1.0f, 0.7f);
    float4 white = float4(0.95f, 1.0f, 1.0f, 0.0f);
    float4 color;

    if (lifeRate < 0.5f)
    {
        color = lerp(blue, purple, lifeRate / 0.5f);
    }
    else
    {
        color = lerp(purple, white, (lifeRate - 0.5f) / 0.5f);
    }

    color.a *= 1.0f - lifeRate;
    gParticles[particleIndex].color = color;

    if (distanceToEmitter < 0.35f ||
